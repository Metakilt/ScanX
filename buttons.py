...
class Shopping:
    ...
    def scan_2(self):
        ...
        if item_name in st.session_state.cart:
            ...
            st.session_state.cart[item_name] = [1*randomiser, item_price, item_price*randomiser]
    ...
    def format(self):
        ...
        if st.session_state.cart:
            for item, details in st.session_state.cart.items():
                st.write(f"{item}, Quantity: {details[0]}, Price: {details[1]}, Total: {details[2]}")
        else:
            st.write("Cart is empty.")


def access_bucket(access_key, access_id, bucket_name, bucket_obj, region):
